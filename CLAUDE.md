...
- **`setRegistryEntry()` is the only place a registry entry's character cap is applied.** `validateAppearanceReply()` and `distillAppearanceTags()` deliberately pass `maxChars: 0`. Capping twice means the second call sees text that already fits, so the entry gets stored as untruncated and the cut goes unreported — which was the original bug. The cap itself comes from `registryMaxChars()` (setting `prompter_registry_max_chars`, clamped 100–2000, default 800) and is stated to the seeding pass from the same function — a stated cap the writer is not held to is worse than none. There is exactly one cap: a `grown` write is held to the same number as a `seed` one. Growth used to have a harder cap of its own on the grounds that a distilled image prompt is a first guess, but truncating a guess yields the same guess with its wardrobe tail missing, frozen into every later request — and the `grown` badge is what marks it as provisional. Any advice about a truncation has to be source-aware: growth never reads the Seeding Instructions, so pointing it at that field is advice that cannot work.
...
- **The output contract lives in `OUTPUT RULES`, not in the user-editable Prompter Instructions**, so a user rewriting their instructions cannot delete the contract. The one relaxation is `prompter_allow_registry_lora`, which swaps the LoRA clause for "copied verbatim from the registry, never invented" and has `validateDirective()` re-insert a pinned call the model dropped — a setting and a mechanism, never a prose exception the user has to argue into `CONSTRAINTS`. **That re-insertion is anchored, not unconditional.** `listRegistryLoraCalls()` pairs each call with the registry tag it renders (`<lora:pubic_hair:1.0>` → `pubic hair`, via `loraAnchorTag()`), and the call is only restored into a prompt that kept its anchor — a call put back on a frame the tag was correctly withheld from makes the renderer paint that anatomy through her clothes, which is the bug the pairing exists for. Whatever decided the tag decides the call with it. A character LoRA matches no tag in its entry, gets anchor `""`, and keeps the unconditional guarantee. Restoration goes *beside the anchor*, not at the end: `PROMPT SHAPE` puts the call in the anatomy block ahead of the wardrobe, and a call appended after the setting tags renders at the weight of a footnote.
